...
namespace xpl {
  ...
  private:
    ...
    inline void process_qualifier(bool qualpublic, bool qualuse, int lvl){
        openTag("qualifier",lvl);
        if(qualpublic){
            ...
            os()<< std::string(lvl, ' ')<<"(none)"<<std::endl;
        }
        closeTag("qualifier",lvl);
        
    }

    inline void process_basic_type(basic_type * type, int lvl){
        if(type!=nullptr){
            os() << std::string(lvl, ' ') << "<basic_type size='" << type->size() << "'>" << std::endl;
            os()<<std::string(lvl,' ')<<type->name()<<std::endl;
            //maybe add translation to actual name (not number). Would envolve a lot more code though.
            process_basic_type(type->subtype(),lvl+2);
            closeTag("basic_type",lvl);
        }
    }
  ...
  protected:
    ...
    void basic_type(basic_type * const type, int lvl);
